...
def evaluate_and_promote(registered_model: str, march_parquet_path: str, challenger_info: dict):
    ...
    print(f"Evaluation on March: Champion RMSE = {champ_rmse:.6f}, Challenger RMSE = {chall_rmse:.6f}")
    ...
    if chall_rmse < champ_rmse:
        ...
        promoted = True
        print(f"Promoted version {challenger_version} to @Champion (was better: {champ_rmse} -> {chall_rmse})")
    else:
        client.set_registered_model_alias(name=registered_model, alias="Challenger", version=challenger_version)
        promoted = False
        print("Champion remains. No promotion.")
...
def challenger_vs_champion(registered_model: str = "workspace.default.nyc-taxi-model-prefect"):
    ...
    EXPERIMENT_NAME = "/Users/monica.ibarra@iteso.mx/nyc-taxi-experiment-prefect"
